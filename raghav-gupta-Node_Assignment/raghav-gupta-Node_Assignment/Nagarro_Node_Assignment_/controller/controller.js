...
exports.teacher_login_check=(req,resp)=>{
    ...
    {   
        ...
        con.query("select * from teachers_db where Email=? and Password=?",data,(error,result)=>{
            if(result==''){
...
exports.teacher_display=(req,resp)=>{
    if(req.session.user!=null){
        con.query("select * from students_db",(error,result)=>{
...
exports.edit_student=(req,resp)=>{
    ...
    con.query("select * from students_db where Roll_no=?",rollno,(error,result)=>{
...
exports.update_student=(req,resp)=>{
    ...
    {  let data = [req.body.Name,req.body.Score,req.body.dob,req.body.Roll_no]
        con.query("update students_db  set  Name=?, Score=?,dob=? where Roll_no=? ",data,(error,result)=>{
            if(error){
                console.error("Error")
            }
            else if(result){
...
exports.save_student=(req,resp)=>{
        ...
        if(errors.isEmpty())
         {
             con.query("Insert into students_db set ?",data,(error,result)=>{
                 if(error){
...
exports.delete_students=(req,resp)=>{
    let rollno  = req.params.rollno;
    con.query("delete from students_db where Roll_no=?",rollno,(error,result)=>{
...
exports.studentresult=(req,resp)=>{
    ...
    con.query("select * from students_db where Roll_no=? And Dob=?",data,(error,result)=>{
     if(result[0]!=""){
